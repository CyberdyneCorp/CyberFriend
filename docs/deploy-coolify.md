...
| `DECISION_MIN_SIMILARITY` | Optional. Cosine a stored decision must reach against the topic of "o que decidimos sobre Y?" to be listed; defaults to `0.4`. Lower it if a multilingual embedding model misses cross-language matches. Declared for `bot`. |
