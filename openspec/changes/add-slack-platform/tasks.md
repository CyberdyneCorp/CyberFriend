...
- [ ] 1.3 Migration: `slack_install` (encrypted tokens), `slack_user`, `slack_channel_membership`, `slack_reconcile_state`
- [ ] 1.4 `SlackIngress`: Socket Mode and HTTP (signature v0, 5-min window, `url_verification`), ack-first worker queue, `event_id` dedup
- [ ] 1.5 Foreign team/enterprise rejection; `app_uninstalled`/`tokens_revoked` purge
- [ ] 1.6 Liveness in `health.py`; secrets in redaction registries
- [ ] 1.7 Tests: forged/stale signature, retry dedup, foreign install, uninstall purge, ack under 3 s with a slow handler
...
- [ ] 2.2 Edits and deletes through the existing tombstone path
- [ ] 2.3 `SlackChatSource.backfill` with `BackfillPage`, token bucket, `Retry-After`, `SLACK_BACKFILL_DAYS`, thread replies
- [ ] 2.4 Retention reconcile job with `SLACK_RECONCILE_DAYS`, `SLACK_RETENTION_DAYS`
- [ ] 2.5 Files: `SlackAttachmentFetcher` (url_private), `file_deleted`/`file_unshared`
- [ ] 2.6 Tests: resume without gaps, 429 handling, `not_in_channel`, deleted message unretrievable, purge reconcile
...
- [ ] 3.3 `SlackAudienceResolver`: im private, mpim GROUP_DIRECT, private/public/Connect/thread rules
- [ ] 3.4 Regression tests at the repository layer: guest, single-channel guest, Connect external user, private non-member, deactivated user, mpim-not-DM, public channel with guest, Grid foreign workspace, Slack unreachable

## 4. PR-S4: Bot surface

- [ ] 4.1 `SlackMentionCodec` (`<@U…>`, `<#C…|name>`, `<!here>`), identifier patterns into the egress guard
- [ ] 4.2 `SlackRenderer` (`markdown_text`, mrkdwn, escaping, defang, split), `SlackPermalinks`
...
- [ ] 4.10 Tests: foreign button press, expired prompt, mpim fact withheld, facts-about-others with `<@U…>`, `<!channel>` defanged, alert delivered to im
...
- [ ] 7.2 No-persistence guarantee test (database empty of Slack content after an answer)
